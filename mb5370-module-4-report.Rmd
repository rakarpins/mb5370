...
### Module 4- Data science: Workshop 1

# 2.4	Installing the tidyverse
First, you’ll need to install the tidyverse. You may get an error if R is out of date, so please work through any of the guidance provided by the error message.
...
For example, if you receive an error about failing to load the tidyverse, try to use remove.packages('rlang') to uninstall it and then reinstall with install.packages('rlang') to get the latest version. You’ll need to restart RStudio as well.

# 2.5	Obtain the data
To provide the strongest foundation in ggplot2 possible, and fully understand its functionality, it’s important to remove any component of data interpretation or uncertainty from the learning experience. We will therefore use the dataset provided with ggplot2, which is particularly designed to help you understand the functionality of the R package without being concerned about data issues, data wrangling (yet!) or even importing the dataset.

The mpg data frame found in ggplot2 is a dataset with observations (234 rows of data) and variables (in the columns). Have a look at the data frame by typing it into the R console.

```{r}
mpg
```
The variables in the mpg data frame include:
- disp, which is the engine size of the cars in litres
- hwy, which is the cars fuel efficiency in miles per gallon (mpg). A car with a low fuel efficiency consumes more fuel than a car with a high fuel efficiency when they travel the same distance.
...
```
# the first line of code gives r the coordinates (otherwise it would give only a grey box)
# aes give the variable to the axis
# plotted engine size vs fuel efficiency
From the R for Data Science textbook (note that all indented text in this manual is directly from this textbook):
...
One thing that can greatly help your ability to develop plots using ggplot2 is to use a template. In all cases, your ggplot code will follow a generic template the same as that below. You will always call ggplot, offer it data, and then provide a geom function with a collection of mappings, which dictate how your plot will look.
...
In addition to simply plotting coordinates of x, y data, you can add a third variable, such as class, by mapping it to an aesthetic. 
...
Change point transparency by class (alpha). Here the alpha aesthetic is what dictates the transparency. Also consider trying a simple number, like 0.3 to this aesthetic.
...
```
# Warning: [38;5;232mUsing alpha for a discrete variable is not advised
...
Warning: [38;5;232mRemoved 62 rows containing missing values (`geom_point()`).[39m
...
# 2.8 Troubleshooting
As with all programming you’ll run into some problems. That’s no worries, it’s very common for data scientists or analysts to have to keep testing and trying (no one gets it first go!). 
...
If you’ve read the scientific literature, you’ll note that a common practice is to break a single complex plot into many sub plots (or panels). This allows you to develop separate plots for a range of reasons, most often to show a subset of your data. 
...
Facet plot on the combination of two variables using facet_grid(). facet_grid needs two variables separated by a ~.
...
So let’s recap. A geom is an object that your plot uses to represent the data. To change the geom type in your plot, simply change the geom function that you add to your plot. It’s sometimes the case you might simply want to try som out, in which case you could use comments given the way we iteratively build ggplots by adding new instructions to the plot statements.
...
```
It might be hard to to understand what’s going on, but here you can see we are fitting a smooth line to different subsets of the data

Here we have two geoms on the same graph (points and lines)! 
ggplot2 has over 40 different geoms! Have a look at the ggplot2 cheatsheet for some examples - all of the below in yellow are different geoms you can use for different things. 


This is getting fairly exciting and shows how flexible ggplot2 can be, but let’s lock in some ideas before moving onto making our own plots with multiple geoms. 

So far we’ve only made fairly simple plots, but what happens if you want o to group objects by a categorical variable (like species or sex or site)? Here we will use the group argument to show that our data is in fact grouped - it won’t add a new legend but can easily be used to vary your plots by the discrete variable.
...
Exactly what we often want to do. However, note how the top and second rows are simply duplicated, meaning if you wanted to change the x variable in your plot, you’d need to change it in several locations?
...
Here’s an example of where we make the the exact same plot as above, but it is programmatically more efficient and makes it easy for you to simply change a variable that you want to plot on the x or y axis. 
```{r}
...
The extra nice thing is that you can still use mappings under ggplot() to reduce duplications in code (like in the above code). This is super handy, particularly if you want to change point styles or develop your own customisations. 

Use mappings in specific layers to enable the display of different aesthetics in different layers (super handy!). Note how the line is note styled by class but you can style the points by themselves.
```{r}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
...
You will learn what exactly filter() does later on in this module, but for now note how it just shows a small subset of the dataset.
...
Now this incredible functionality of ggplot2 will open up a wealth of ways you can visualise your data, without the need to fit statistical models or summarise your dataset! It’s hard to describe how time-saving this can be, especially since this feature is as flexible as the rest of ggplot2.
...
But like in general R and even ArcGIS, it’s important to understand the defaults are not the only thing you can do. Indeed, that defaults are there in the first place can have big implications for your results, so you should always make an effort to understand any of these ‘black box’ outputs. 
...
Add don’t worry yet about tribbles, we will learn what a tribble is soon. Basically a dataframe.
```{r}
demo <- tribble(
  ~cut,         ~freq,
  "Fair",       1610,
...
You might also want to show a little more about these transformations in your plot, which is good practive to be transparent about uncertainty or any other limitation of your data. 
...
Now here’s another way to boost the way you can convey information with plots using ggplot2. You can use colour or fill to change aspects of bar colours.
```{r}
ggplot(data = diamonds) + 
...
```
And now, try using these aesthetics to colour by another variable like clarity. Notice how the stacking is done automatically. This is done behind the scenes with a position argument.
...
Now that we have the strongest foundation possible in understanding how ggplot2 allows us to flexibly build just about any graph we want, we can begin to focus on the aspects of data viz that make an ok plot an excellent plot. Previously, our plots were useful for data exploration, but now we want to start focusing on how we communicate our results in an effective and repeatable manner. 
This part of data viz allows you to bring in your own creativity and, as such, can be fun, slightly addictive, and will enable you to meet your aspirations of building quality outputs from any programmed workflow you now develop. 
Now it’s time to learn the customisation functions available to you in ggplot2. Some of these require skills you’ll learn in the data wrangling parts of this module, so if anything pops up that you don’t yet understand, that’s ok, we’ll cover it soon.
...
```
There are a few more things you can do with labels, including mathematical notations and superscripts, read the ggplot2 help for the function to find out.

# 3.2 Annotations
But what if you want to add text to the plot itself? Sometimes you’ll want to do this to highlight a single observation, for instance. Here you can use geom_text() to add textual labels to your plots. This works in the same way as geom_point() but rather than a shape geometry it can add a label. 
...
# 3.3 Scales
Changing the default scales on ggplot2 can also help you customise your plots and allow improved communication of your results. 
...
However, if you want to tweak them you can do it by offering values to the scale parameters, simply try adding numbers in the appropriate scale arguments.
...
# 3.7 Themes
And finally, now you can customise the entire theme of your plot. Themes allow you to change some or all of the non-data elements of your plot with a theme.
...
Export data as a csv from QFISH website and import it into figs < mb5370 < Github < MB5370(desktop). Then in rstudio import the dataset. 
Shark Control Program- Number of animals caught by length category within the shark control areas.
...
```{r}
ggplot(data = QFISH_Sheet1) + 
  geom_point(mapping = aes(x = Area, y = SharksCounted, colour = LengthCategory))

```

```{r}
ggplot(data = QFISH_Sheet1) + 
  geom_bar(mapping = aes(x = LengthCategory, y = SharksCounted, fill = LengthCategory), stat = 'identity', show.legend = FALSE) + 
  facet_wrap(~ Area, nrow = 3) +
  labs(title = "Total number caught by area and length category",
       x = "Size Category",
       y = "Sharks Counted") +
  theme(legend.position = "bottom") +
  theme_linedraw()
...
```{r}
aus <- st_read("copepod-data/data-for-course/spatial-data/Aussie/Aussie.shp")
```
```{r}
shelf <- st_read("copepod-data/data-for-course/spatial-data/aus_shelf/aus_shelf.shp")
```
